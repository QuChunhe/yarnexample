...
public abstract class ContainerBuilder {
    ...
    private LocalResource buildLocalResource(FileSystem fs, Path jarPath) throws IOException {
    ...
    protected Map<String, String> buildEnvironment() {
        ...
        for (String cpEntry : conf.getStrings(YarnConfiguration.YARN_APPLICATION_CLASSPATH,
            ...
            classPathEnv.append(ApplicationConstants.CLASS_PATH_SEPARATOR).append(cpEntry.trim());
        }
        ...
        try {
            ...
            while (files.hasNext()) {
                ...
                try {
                    ...
                    log.error("Cannot add " + jarPath.toString() + " to LocalResource map!", e);
                }
    ...
    private List<String> buildExecCommand() {
        ...
        String cmd = "${JAVA_HOME}/bin/java " + "-cp .:${CLASSPATH} " + "-Xmx" + maxHeapSize + "M " + "-Xms"
                + minHeapSize + "M " + className + " " + getInputArguments() + " " + "1>"
                + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/" + outlog + " " + "2>"
                + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/" + errlog + " ";
