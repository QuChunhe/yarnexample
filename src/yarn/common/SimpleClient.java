...
public class SimpleClient {
	...
	private Resource checkResourceAvailable(YarnClientApplication app) {
	    ...
	    if ((requireVCores < 0) || (requireVCores > maxVCores)) {
	    	log.warn(Constant.AM_REQUIRED_VIRTUAL_CORES +" is not setted or is greated than the max virtual cores!");
	...
	private void printFinalJobReport() {
		...
		try {
			...
			log.error("Exception encountered while attempting to request "
							+ "a final job report for " + jobName, e);
			return;
		}
		...
		log.info("Completed " + jobName + " : " + finalAppStatus.name() + 
	...
	public boolean doesFinish() {
		...
		if (null == appId) {
			...
			try {
				...
				switch (state) {
				case FAILED:
					log.warn(jobName + " reports FAILED state, diagnostics show: " + 
